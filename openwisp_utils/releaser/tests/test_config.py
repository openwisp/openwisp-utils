...
def test_npm_package_invalid_version(
    ...
    """Tests that npm package handles invalid version format."""
...
def test_docker_package_invalid_version(
    ...
    with pytest.raises(ValueError, match="does not have expected 3 parts"):
        load_config()
...
def test_ansible_package_malformed_version(
    ...
    """Tests ansible package with malformed version string."""
    create_ansible_lint(project_dir)
...
# OpenWRT Agents Package Tests
def test_openwrt_agents_package_detection(
    project_dir, create_luacheckrc, create_version_file, create_changelog, init_git_repo
):
    """Tests that openwrt-agents package type is detected when .luacheckrc exists."""
    ...
    assert config["package_type"] == "openwrt-agents"
    assert config["version_path"] == "VERSION"
    assert config["CURRENT_VERSION"] == [1, 2, 3, "final"]


def test_openwrt_agents_without_version_file(
    project_dir, create_luacheckrc, create_changelog, init_git_repo
):
    """Tests openwrt-agents package without VERSION file."""
    create_luacheckrc(project_dir)
    create_changelog(project_dir)
    init_git_repo(project_dir)
...
def test_openwrt_agents_invalid_version(
    project_dir, create_luacheckrc, create_changelog, init_git_repo
):
    """Tests openwrt-agents package with invalid version format."""
    create_luacheckrc(project_dir)
