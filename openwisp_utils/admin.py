...
class CopyableFieldsAdmin(UUIDAdmin):
    ...
    def get_fields(self, request, obj=None):
        ...
        # We should exclude `copyable_fields` fields when the object
        # doesn't exist for example, in the case of `add_view` because
        # `copyable_fields` can't be edited and are auto generated by the system
        if not obj:
